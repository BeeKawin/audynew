...
class _EmotionClassifyScreenState extends State<EmotionClassifyScreen> {
  ...
  Future<void> _continueAfterAnswerFeedback(
    AudyController controller, {
    required bool isCorrect,
  }) async {
    final praiseIndex =
        (controller.classifyCurrentRound - 1) % _praiseSoundOrder.length;
    final praiseSoundPath = _praiseSoundOrder[praiseIndex];

    if (isCorrect) {
      await SoundService.instance.playCorrectThenEmotionTransition();
    } else {
    ...
    await Navigator.push<bool>(
      ...
      MaterialPageRoute(
        builder: (_) => SelfieCaptureScreen(
          ...
          successPraiseSoundPath: praiseSoundPath,
        ),
      ),
    );
